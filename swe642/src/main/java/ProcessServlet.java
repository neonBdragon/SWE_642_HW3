...
public class ProcessServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		studentParts = StudentDAO.getData(request.getParameter("user_choice"));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		studentParts = StudentDAO.getData(request.getParameter("student_id"));
		if(!studentParts.isEmpty())
		{
		...
		DataBean beanData = new DataBean();
		beanData.setMean(mean);
		beanData.setSTDDev(stdDev);
		session.setAttribute("databean", beanData);
		
		logger.info("Dispatching!");
		if(mean >= 90.0) request.getRequestDispatcher("WinnerAcknowledgement.jsp").forward(request, response);
		else request.getRequestDispatcher("SimpleAcknowledgement.jsp").forward(request, response);
	}
