 ...
 * 
 * 
 * StudentDAO class  that encapsulates code to store and retrieve the Survey data into/from a database. It provides two methods: saveForm, retrieveForm

 */
public class StudentDAO {
	private final static String DRIVER_NAME = "oracle.jdbc.driver.OracleDriver";
	private final static String DB_URL = "jdbc:oracle:thin:@artemis.vsnet.gmu.edu:1521/vse18c.vsnet.gmu.edu";
	private final static String USERNAME = "bmack4";
	private final static String PASSWORD = "thootcho";
	
	private static String querySET = "INSERT INTO STUDENT (sid, sname, address, city, state, zipcode, telephone, email, url, dos) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, TO_DATE(?, 'YYYY-MM-DD'))";
	private static String queryGET = "SELECT sid, sname, address, city ,state, zipcode, telephone, email, url, dos FROM STUDENT where sid = ?";
	private static String queryGETALL = "SELECT sid FROM STUDENT";
	...
	public static List<String> saveData(List<String> data) {
		...
		try {
			...
			myStmt.setString(10, data.get(9));					//date_of_survey
